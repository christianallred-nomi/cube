...
export class BigqueryQuery extends BaseQuery {
  ...
  private formatInterval(interval: string): string {
    ...
    if (intervalParsed.year && intKeys === 1) {
      ...
      return `'${intervalParsed.year}-${intervalParsed.month}' YEAR TO MONTH`;
    } else if (intervalParsed.year && intervalParsed.month && intervalParsed.day && intKeys === 3) {
      return `'${intervalParsed.year}-${intervalParsed.month} ${intervalParsed.day}' YEAR TO DAY`;
    } else if (intervalParsed.year && intervalParsed.month && intervalParsed.day && intervalParsed.hour && intKeys === 4) {
      return `'${intervalParsed.year}-${intervalParsed.month} ${intervalParsed.day} ${intervalParsed.hour}' YEAR TO HOUR`;
    } else if (intervalParsed.year && intervalParsed.month && intervalParsed.day && intervalParsed.hour && intervalParsed.minute && intKeys === 5) {
      return `'${intervalParsed.year}-${intervalParsed.month} ${intervalParsed.day} ${intervalParsed.hour}:${intervalParsed.minute}' YEAR TO MINUTE`;
      ...
      return `'${intervalParsed.year}-${intervalParsed.month} ${intervalParsed.day} ${intervalParsed.hour}:${intervalParsed.minute}:${intervalParsed.second}' YEAR TO SECOND`;
      ...
      return `'${intervalParsed.month} ${intervalParsed.day}' MONTH TO DAY`;
    } else if (intervalParsed.month && intervalParsed.day && intervalParsed.hour && intKeys === 3) {
      return `'${intervalParsed.month} ${intervalParsed.day} ${intervalParsed.hour}' MONTH TO HOUR`;
    } else if (intervalParsed.month && intervalParsed.day && intervalParsed.hour && intervalParsed.minute && intKeys === 4) {
      return `'${intervalParsed.month} ${intervalParsed.day} ${intervalParsed.hour}:${intervalParsed.minute}' MONTH TO MINUTE`;
      ...
      return `'${intervalParsed.month} ${intervalParsed.day} ${intervalParsed.hour}:${intervalParsed.minute}:${intervalParsed.second}' MONTH TO SECOND`;
      ...
      return `'${intervalParsed.day} ${intervalParsed.hour}' DAY TO HOUR`;
    } else if (intervalParsed.day && intervalParsed.hour && intervalParsed.minute && intKeys === 3) {
      return `'${intervalParsed.day} ${intervalParsed.hour}:${intervalParsed.minute}' DAY TO MINUTE`;
    } else if (intervalParsed.day && intervalParsed.hour && intervalParsed.minute && intervalParsed.second && intKeys === 4) {
      return `'${intervalParsed.day} ${intervalParsed.hour}:${intervalParsed.minute}:${intervalParsed.second}' DAY TO SECOND`;
    } else if (intervalParsed.hour && intervalParsed.minute && intKeys === 2) {
      return `'${intervalParsed.hour}:${intervalParsed.minute}' HOUR TO MINUTE`;
    } else if (intervalParsed.hour && intervalParsed.minute && intervalParsed.second && intKeys === 3) {
      return `'${intervalParsed.hour}:${intervalParsed.minute}:${intervalParsed.second}' HOUR TO SECOND`;
    } else if (intervalParsed.minute && intervalParsed.second && intKeys === 2) {
      return `'${intervalParsed.minute}:${intervalParsed.second}' MINUTE TO SECOND`;
    }
  ...
  public subtractInterval(date, interval) {
    return `DATETIME_SUB(${date}, INTERVAL ${this.formatInterval(interval)})`;
  }

  public addInterval(date, interval) {
    return `DATETIME_ADD(${date}, INTERVAL ${this.formatInterval(interval)})`;
  }

  public subtractTimestampInterval(date, interval) {
    return `TIMESTAMP_SUB(${date}, INTERVAL ${this.formatInterval(interval)})`;
  }

  public addTimestampInterval(date, interval) {
    return `TIMESTAMP_ADD(${date}, INTERVAL ${this.formatInterval(interval)})`;
  }
